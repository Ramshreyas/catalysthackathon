...
CONTRACT nft : public eosio::contract {
	...
	ACTION setrampayer(name payer, id_type id);
        ...
        using token_index = eosio::multi_index<"token"_n, token, indexed_by< "byowner"_n, const_mem_fun< token, uint64_t, &token::get_owner>>, indexed_by< "bysymbol"_n, const_mem_fun< token, uint64_t, &token::get_symbol>>, indexed_by<"byuri"_n, const_mem_fun< token, uint64_t, &token::get_uri>>>;
